...
while read -p "$(echo -e "\n${RED}Would you like to install some gnome-extras? (yes/no): ${NC}")" EXTRAS; do
    case "$EXTRAS" in
        [Yy] | [Yy][Ee][Ss])
            command sudo apt-get install -qq -y dconf-editor gnome-shell-extension-manager input-remapper diodon xournalpp || error-echo "installing extras"
            echo "Installed the following extras: dconf-editor, gnome-shell-extension-manager, input-remapper, diodon, xournalpp"
            break;;
        [Nn] | [Nn][Oo])
